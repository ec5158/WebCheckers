...
import com.webcheckers.ui.turn.PostBackupMoveRoute;
import com.webcheckers.ui.turn.PostCheckTurnRoute;
import com.webcheckers.ui.turn.PostSubmitTurnRoute;
...
public class WebServer {
  ...
  public static final String ERROR_INGAME = "error_ingame";
   ...
   */
  public WebServer(final GameCenter gameCenter, TemplateEngine templateEngine, final Gson gson) {
    // validation
  ...
  public void initialize() {
    ...
    // PlayerLobby obj stores information about players online
    PlayerLobby playerLobby = gameCenter.getPlayerLobby();
    Matchmaking matchmaking = gameCenter.getMatchmaking();

    // Home route with sign-in link, player list, etc.
    get(HOME_URL, new GetHomeRoute(gameCenter, playerLobby, templateEngine));
    // Sign-in page
    get(SIGNIN_URL, new GetSigninRoute(gameCenter, templateEngine));
    // Sign-in attempt from sign-in page
    post(SIGNIN_URL, new PostSigninRoute(gameCenter, playerLobby, templateEngine));
    // Sign-out attempt
    post(SIGNOUT_URL, new PostSignOutRoute(gameCenter, templateEngine));
    // Attempt to start game -> show game board
    get(GAME_URL, new GetGameRoute(gameCenter, playerLobby, matchmaking, templateEngine));
    // Validate moves
    post(VALIDATE_MOVE_URL, new PostValidateMoveRoute(matchmaking));

    post(CHECK_TURN_URL, new PostCheckTurnRoute(matchmaking));

    post(BACKUP_MOVE_URL, new PostBackupMoveRoute(matchmaking));

    post(SUBMIT_TURN_URL, new PostSubmitTurnRoute(matchmaking));
